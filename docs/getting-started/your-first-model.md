...
3. Pressione `Tab` no teclado para alternar do **Modo Objeto** para o **Modo de Edição**.
4. Observe que na barra da viewport surgem os botões de seleção de componentes: `⬝ Vértice (1)`, `╱ Aresta (2)` e `▨ Face (3)`.
...
## Passo 5: Suavizando Cantos com Chanfro (Bevel)
1. Pressione a tecla `2` para entrar no modo de seleção de **Arestas**.
2. Selecione as arestas dos cantos externos do caixote segurando `Shift`.
3. Pressione `Ctrl+B` (ferramenta **Chanfro / Bevel**).
...
1. Pressione `Tab` para sair do Modo de Edição e voltar ao **Modo Objeto**.
