...
pub(crate) fn bevel_edge(mesh: &Mesh, a: u32, b: u32, amount: f32) -> Option<Mesh> {
    ...
    for (fi, face) in mesh.faces.iter().enumerate() {
        ...
        for (k, &vertex) in face.verts.iter().enumerate() {
            ...
            for neighbor in [prev, next] {
                let neighbor_vertex = face.verts[neighbor];
                if neighbor_vertex == a || neighbor_vertex == b {
                    continue;
                }
                let &(offset, t) = offsets.get(&(vertex, neighbor_vertex))?;
                vertices.push(offset);
                uvs.push([
                    face.uv[k][0] + (face.uv[neighbor][0] - face.uv[k][0]) * t,
                    face.uv[k][1] + (face.uv[neighbor][1] - face.uv[k][1]) * t,
                ]);
            }
        }
    ...
    let indices = if face.verts[(k + 1) % face.verts.len()] == b {
        vec![b1, a1, a2, b2]
    } else {
        vec![a1, b1, b2, a2]
    };
